...
if uploaded_file:
    ...
    # ✅ กราฟรวม
    st.subheader("📈 กราฟรวมอัตราสึกหรอเฉลี่ยของ Brush")
    fig_combined = go.Figure()
    fig_combined.add_trace(go.Scatter(x=x_vals, y=y_upper, mode='lines+markers+text',
                                      name='Upper Avg Rate', line=dict(color='red'),
                                      text=[str(i) for i in x_vals], textposition='top center'))
    fig_combined.add_trace(go.Scatter(x=x_vals, y=y_lower, mode='lines+markers+text',
                                      name='Lower Avg Rate', line=dict(color='darkred'),
                                      text=[str(i) for i in x_vals], textposition='top center'))
    fig_combined.update_layout(title='📈 Average Wear Rate per Brush (1–32)',
                               xaxis_title='Brush Number', yaxis_title='Wear Rate (mm/hour)',
                               template='plotly_white')
    st.plotly_chart(fig_combined, use_container_width=True)

    # ✅ กราฟ Upper
    st.subheader("🔺 กราฟ Upper Avg Rate")
    fig_upper = go.Figure()
    fig_upper.add_trace(go.Scatter(x=x_vals, y=y_upper, mode='lines+markers+text',
                                   name='Upper Avg Rate', line=dict(color='red'),
                                   text=[str(i) for i in x_vals], textposition='top center'))
    fig_upper.update_layout(title='🔺 Upper Average Wear Rate per Brush',
                            xaxis_title='Brush Number', yaxis_title='Wear Rate (mm/hour)',
                            template='plotly_white')
    st.plotly_chart(fig_upper, use_container_width=True)

    # ✅ กราฟ Lower
    st.subheader("🔻 กราฟ Lower Avg Rate")
    fig_lower = go.Figure()
    fig_lower.add_trace(go.Scatter(x=x_vals, y=y_lower, mode='lines+markers+text',
                                   name='Lower Avg Rate', line=dict(color='darkred'),
                                   text=[str(i) for i in x_vals], textposition='top center'))
    fig_lower.update_layout(title='🔻 Lower Average Wear Rate per Brush',
                            xaxis_title='Brush Number', yaxis_title='Wear Rate (mm/hour)',
                            template='plotly_white')
