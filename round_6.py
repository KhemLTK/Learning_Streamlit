...
if uploaded_file:
    ...
    # ==== TABLE: Upper Only with red highlight on Avg Rate ====
    st.subheader("📋 ตาราง Avg Rate - Upper")
    def highlight_upper(s):
        return ['color: red; font-weight: bold' if col == "Avg Rate (Upper)" else '' for col in s.index]

    st.dataframe(upper_df.style.apply(highlight_upper, axis=1).format("{:.6f}"), use_container_width=True)

    # ==== TABLE: Lower Only ====
    st.subheader("📋 ตาราง Avg Rate - Lower")
    st.dataframe(lower_df.style.format("{:.6f}"), use_container_width=True)

    # ==== GRAPH 1: Combined Plot ====
    ...
    # ==== GRAPH 2: Remaining Life Barplot ====
