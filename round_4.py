...
if uploaded_file:
    ...
    # ===== STEP 1: Calculate wear rates from sheets =====
    ...
    # ===== STEP 2: Extract current lengths from Sheet7 =====
    ...
    # ===== STEP 3: Display table and plotly graph for Avg Rate =====
    ...
    st.subheader("📋 ตารางอัตราสึกหรอเฉลี่ย (Avg Rate)")
    st.dataframe(combined_df.style.format("{:.6f}"), use_container_width=True)

    fig_combined = go.Figure()
    fig_combined.add_trace(go.Scatter(
        x=brush_numbers, y=avg_rate_upper, mode='lines+markers+text',
        name='Upper Avg Rate', line=dict(color='red'),
        text=[str(i) for i in brush_numbers], textposition='top center'
    ))
    fig_combined.add_trace(go.Scatter(
        x=brush_numbers, y=avg_rate_lower, mode='lines+markers+text',
    ...
    # ===== STEP 4: Matplotlib bar chart for remaining hours =====
    ...
    fig_upper.update_layout(
        title='🔺 Upper Average Wear Rate per Brush',
        xaxis_title='Brush Number',
        yaxis_title='Wear Rate (mm/hour)',
        template='plotly_white'
    )
    ...
    fig_lower.update_layout(
        title='🔻 Lower Average Wear Rate per Brush',
        xaxis_title='Brush Number',
        yaxis_title='Wear Rate (mm/hour)',
        template='plotly_white'
    )
    ...
    # === Plot 4: Remaining Hours Bar Plot
    st.subheader("⏳ ชั่วโมงที่เหลือก่อนถึง 35mm (Remaining Life Estimate)")
